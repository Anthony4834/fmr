...
export const metadata: Metadata = {
  title: 'Browse cities | fmr.fyi',
  description: 'Browse cities by state to view HUD Fair Market Rent (FMR/SAFMR) data.',
  alternates: { canonical: 'https://fmr.fyi/cities' },
};
  ...
  return (
    <main className="min-h-screen bg-[#fafafa] antialiased">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <h1 className="mt-6 text-3xl font-bold tracking-tight text-[#0a0a0a]">Browse cities</h1>
        <p className="mt-3 text-base text-[#525252]">
          Pick a state to browse cities. Each city page redirects to the main FMR view.
        </p>
        ...
        <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
          {states.rows.map((row: any) => (
            <a
              key={row.state_code}
              href={`/cities/${String(row.state_code).toLowerCase()}`}
