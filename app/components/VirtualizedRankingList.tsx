...
export default function VirtualizedRankingList({
  ...
  if (error) {
    return (
      <div className="flex-1 flex items-center justify-center p-4 text-xs text-[#991b1b]">
        {error}
      </div>
    );
  }
  ...
  if (loading && items.length === 0) {
    return (
      <div className="flex-1 divide-y divide-[#e5e5e5] overflow-y-auto custom-scrollbar">
        {[...Array(10)].map((_, i) => (
  ...
  if (!loading && items.length === 0) {
    return (
      <div className="flex-1 flex items-center justify-center p-4 text-xs text-[#737373]">
        No results found
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto custom-scrollbar">
      {items.map((item) => {
        ...
        return (
          <a
            key={`${type}-${item.rank}`}
            href={getHref(item)}
            className="block px-3 sm:px-4 py-2 sm:py-2.5 hover:bg-[#fafafa] transition-colors border-b border-[#e5e5e5]"
          >
            <div className="flex items-start justify-between gap-2 sm:gap-3">
              <div className="flex items-start gap-2 sm:gap-2.5 min-w-0 flex-1">
      ...
      {hasMore && (
        ...
        >
          <div className="h-4 w-4 rounded-full border-2 border-[#d4d4d4] border-t-transparent animate-spin mx-auto" />
        </div>
      )}

      {/* End of results */}
      {!hasMore && items.length > 0 && (
        <div className="px-3 sm:px-4 py-2 text-xs text-[#737373] text-center">
          End of results
        </div>
      )}
    </div>
